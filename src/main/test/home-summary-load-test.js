...
const HOME_SUMMARY_URL = `${BASE_URL}/api/v1/home/summary`;
...
const AUTH_MODE = (__ENV.AUTH_MODE || "guest").toLowerCase();
...
function buildOptions() {
  const baseOptions = {
    ...
    thresholds: {
      "http_req_failed{endpoint:home_summary}": ["rate<0.01"],
      "http_req_duration{endpoint:home_summary}": ["p(95)<500", "p(99)<1000"],
    },
  };
...
export function setup() {
  ...
  if (AUTH_MODE !== "user") {
    fail("AUTH_MODE must be guest or user.");
  }
...
export default function (data) {
  const session = data.sessions.length > 0
    ? data.sessions[(__VU - 1) % data.sessions.length]
    : null;
  const requestId = `k6-home-summary-${__VU}-${__ITER}-${Date.now()}`;
  const url = `${HOME_SUMMARY_URL}?season=${encodeURIComponent(SEASON)}`;
  ...
  const response = http.get(url, {
    ...
    tags: {
      ...
      season: SEASON,
      ...(session ? { user_index: session.userIndex } : {}),
    },
  });

  check(response, {
    "home summary status is 200": (res) => res.status === 200,
    "home summary has todayFixtures array": (res) => hasArrayField(res, "todayFixtures"),
    "home summary has standings array": (res) => hasArrayField(res, "standings"),
    "user response has favorites object": (res) => AUTH_MODE !== "user" || hasObjectField(res, "favorites"),
    "guest response has empty favorites": (res) => AUTH_MODE !== "guest" || hasEmptyFavorites(res),
    "home summary response is under 500ms": (res) => res.timings.duration < 500,
  });
...
function hasObjectField(response, fieldName) {
  try {
    const body = response.json();
    return typeof body[fieldName] === "object" && body[fieldName] !== null && !Array.isArray(body[fieldName]);
  } catch (error) {
    return false;
  }
}

function hasEmptyFavorites(response) {
  try {
    const body = response.json();
    return Array.isArray(body.favorites?.teams)
      && body.favorites.teams.length === 0
      && Array.isArray(body.favorites?.players)
      && body.favorites.players.length === 0;
  } catch (error) {
    return false;
  }
}
